...
class CianSpiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        # Извлечение URL-ов квартир
        flat_urls = response.css(
            'div._93444fe79c--container--kZeLu._93444fe79c--agent-cont--iStzo a._93444fe79c--link--eoxce::attr(href)').getall()
        for url in flat_urls:
            yield scrapy.Request(url, callback=self.parse_flat)

    def parse_flat(self, response):
        # Обработка каждой страницы квартиры
